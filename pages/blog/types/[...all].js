...
function BlogTypes({categories,categoryId, blogs, total,page,pageSize}) {
                    ...
                    return <div key={category.categoryId}>
                        <Link href={`/blog/types/${category.categoryId}`}>
                            ...
                            <Icon className={`back ${isActive && 'active'}`}
                                  title='展开'
                                  onClick={(e) => {e.stopPropagation();openCategoryChildren(category.categoryId)}}/>
    ...
    const getPageItem=(page,type)=>{
        return <PaginationItem href={`/blog/types/${categoryId}/p${page}/t${total}`}
                               onClick={loading}
                               page={page}
                               type={type}/>
    }
    return <div className='xl-blog-type-main'>
        ...
        <div className='xl-blog-type-blog-list'>
            ...
            {total/pageSize>1&&<Pagination itemRender={getPageItem} defaultCurrent={page} total={total} defaultPageSize={pageSize}/>}
        </div>
    </div>
}
